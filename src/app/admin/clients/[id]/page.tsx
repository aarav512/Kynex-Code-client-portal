...
export default function AdminClientDetailPage() {
  ...
  const { loading, error, data } = usePortalData(async (supabase) => {
    ...
    const [hydrated] = await hydrateClientContacts(supabase, [client]);
    ...
    return { client: hydrated, projects: projects.data ?? [], files: files.data ?? [], payments: payments.data ?? [] };
  });
    ...
    <PortalState loading={loading} error={error}>
      {data?.client ? (
        <div className="space-y-6">
          ...
          <PageHeader title={String(data.client.company_name || 'Client')} description={`${data.client.contact_name || ''} · ${data.client.email || ''}`.trim()} action={<StatusPill status={data.client.status} />} />
