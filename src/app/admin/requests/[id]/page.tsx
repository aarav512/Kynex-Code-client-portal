...
export default function AdminRequestDetailPage() {
  ...
  const { loading, error, data } = usePortalData(async (supabase) => {
    const { data: request } = await supabase.from('requests').select('*, clients!inner(company_name)').eq('id', id).maybeSingle();
    ...
    <PortalState loading={loading} error={error}>
      {data?.request ? (
        <div className="space-y-6">
          ...
          <PageHeader title={data.request.subject} description={`From: ${(data.request as { clients: { company_name: string } }).clients.company_name}`} action={<StatusPill status={data.request.status} />} />
